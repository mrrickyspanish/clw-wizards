...
function PhotoRiverRow({ photos }: { photos: typeof CLUB_PHOTOS }) {
  ...
  return (
    <div className="marquee-group marquee-mask w-full overflow-hidden">
      <div
        className="marquee-track flex w-max gap-2.5 will-change-transform sm:gap-4"
        style={{ animationDuration: '56s' }}
      >
        {groups.map((group, groupIndex) => (
          <div key={groupIndex} className="flex shrink-0 gap-2.5 pr-2.5 sm:gap-4 sm:pr-4">
            {group.map((photo) => (
              <div
                key={`${groupIndex}-${photo.src}`}
                className="relative h-32 w-56 shrink-0 overflow-hidden border border-clw-gold/20 bg-clw-black-2 shadow-xl shadow-black/20 sm:h-40 sm:w-64 lg:h-40 lg:w-72 2xl:h-52 2xl:w-96"
              >
...
export function HomeFacebookSection() {
  return (
    <section className="relative isolate overflow-hidden border-y border-clw-gold/25 bg-clw-black py-10 text-clw-white sm:py-12 lg:py-14 2xl:py-20">
      ...
      <div id="facebook" className="relative mx-auto grid max-w-7xl scroll-mt-24 gap-6 px-5 sm:gap-8 sm:px-8 lg:grid-cols-[0.78fr_1.22fr] lg:items-start lg:gap-10 lg:px-12 xl:px-16 2xl:gap-14 2xl:px-20">
        <div className="text-center lg:sticky lg:top-44 lg:text-left">
          <p className="font-cond text-sm uppercase tracking-[0.28em] text-clw-gold">Social</p>
          <h2 className="mt-3 max-w-3xl uppercase leading-[0.92] text-clw-white lg:max-w-none">
            <span className="block font-cond text-[clamp(2.5rem,10.5vw,5rem)] font-light tracking-[-0.04em] lg:text-[3.7rem] xl:text-[4.2rem] 2xl:text-[5rem]">
              Follow us on
            </span>
            <span className="block font-display text-[clamp(2.75rem,11.5vw,5.6rem)] font-black tracking-[-0.035em] text-clw-gold lg:text-[4rem] xl:text-[4.6rem] 2xl:text-[5.6rem]">
              Facebook
            </span>
          </h2>
          <p className="mx-auto mt-3 max-w-xl text-base leading-relaxed text-clw-gray sm:text-lg lg:mx-0 lg:text-lg lg:leading-[1.65] 2xl:mt-6 2xl:text-xl">
      ...
      <div className="relative mt-6 sm:mt-8 2xl:mt-10">

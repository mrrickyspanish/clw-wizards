...
export function HomeSupportIntro() {
  return (
    <section className="section-light relative overflow-hidden border-y border-clw-gold/35 bg-[#F7F3E9] px-5 py-14 text-clw-ink sm:px-8 sm:py-16 lg:px-12 lg:py-20 xl:px-16 2xl:px-20">
      <div className="absolute inset-0 opacity-65 [background-image:radial-gradient(circle_at_80%_5%,rgba(240,192,32,.16),transparent_24%),linear-gradient(180deg,rgba(255,255,255,.62),transparent_44%)]" />
      <span
        aria-hidden
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="mt-9 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {SUPPORT_OPTIONS.map(({ title, body, href, cta, Icon }) => (
            <Link
              key={title}
              href={href}
              className="group flex min-h-[210px] flex-col border border-clw-gold-dim/35 bg-[#FFFBF3] p-5 text-clw-ink shadow-xl shadow-clw-black/5 transition hover:-translate-y-1 hover:border-clw-gold hover:bg-white sm:min-h-[230px] sm:p-6"
            >
              <div className="flex items-center justify-between gap-6">
                <span className="flex h-12 w-12 items-center justify-center rounded-full bg-clw-gold/12 text-clw-gold-ink ring-1 ring-clw-gold-dim/20">
                  <Icon className="h-7 w-7" strokeWidth={2.35} />
                </span>
              </div>
              <h3 className="mt-7 font-display text-4xl uppercase leading-none tracking-wide text-clw-ink sm:text-5xl lg:text-4xl xl:text-5xl">
                {title}
              </h3>
              <p className="mt-4 text-base leading-relaxed text-clw-muted-dark">
                {body}
              </p>
              <span className="mt-auto pt-6 font-cond text-base uppercase tracking-[0.2em] text-clw-gold-ink transition group-hover:text-clw-ink sm:text-lg lg:text-base xl:text-lg">
                {cta} →
              </span>
            </Link>
          ))}
        ...
        <div className="mt-8 flex justify-start border-t border-clw-gold-dim/30 pt-7">

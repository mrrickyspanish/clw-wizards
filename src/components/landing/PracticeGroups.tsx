...
export function PracticeGroups() {
  return (
    <div className="chamfer-md card-depth flex h-full flex-col border border-clw-gold/10 bg-clw-black-2 p-6">
      <h2 className="font-display text-2xl uppercase tracking-wide text-clw-white">Practice groups</h2>
      ...
      <div className="mt-5 grid flex-1 grid-cols-1 gap-2 sm:grid-cols-2">
        {ORG.practiceGroups.map((group) => (
          <Link
            key={group}
            href="/signup"
            className="group flex items-center gap-3 rounded-md border border-clw-gold/10 bg-clw-black/40 px-3 py-3 transition-colors hover:border-clw-gold/40 hover:bg-clw-black/70"
          >
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-clw-gold/10 text-clw-gold-ink">
              <Shield className="h-4 w-4" />
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-medium text-clw-white">{group}</span>
